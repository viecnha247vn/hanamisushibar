...
function lines(o) {
  ...
  return {
    ...
    items: o.items.map(i => [`${String(i.qty || "").padStart(2)}  ${i.name}`, i.price ? String(i.qty * i.price) : ""]),
    total: kr(o.total),
    payment: `Betalning: ${o.payment}${isTable ? " (betalas vid utgång)" : ""}`,
...
export function starMarkup(o) {
  ...
  for (const [l, r] of L.items) out.push(`[column: left: ${escP(l)}; right: ${escP(r)}]`);
  ...
  out.push("-".repeat(WIDTH), `[bold: on][column: left: Summa; right: ${escP(L.total)}][bold: off]`, esc(L.payment));
...
export function plainText(o) {
  ...
  out.push("=".repeat(WIDTH), ...L.items.map(([l, r]) => row(l, r)));

...
export function starMarkup(o) {
  ...
  if (L.tip) out.push("-".repeat(WIDTH), `[column: left: Dricks; right: ${escP(L.tip)}]`);
  out.push(L.tip ? "" : "-".repeat(WIDTH), `[bold: on][column: left: Summa; right: ${escP(L.total)}][bold: off]`,
...
export function plainText(o) {
  ...
  if (L.tip) out.push("-".repeat(WIDTH), row("Dricks", L.tip), row("Summa", L.total));
  else out.push("-".repeat(WIDTH), row("Summa", L.total));
  out.push(L.payment);

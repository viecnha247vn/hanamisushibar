...
export function mixPrice(id, opts) {
  ...
  let swaps = 0;
  for (const k of choices) swaps += Math.max(0, (counts[k] || 0) - (cfg.nigiri[k] || 0));
  ...
  if (swaps) parts.push("Nigiri: " + choices.filter(k => counts[k]).map(k => `${counts[k]} ${counts[k] > 1 && MIX.plural[k] || k}`).join(", "));

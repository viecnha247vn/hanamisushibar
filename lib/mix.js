 ...
 * - Egen hosomaki (choice): gästen väljer en fyllning, utan tillägg.
...
export const MIX = {
  ...
  items: {
    ...
    // Egen hosomaki: välj fyllning
    "maki-7": { choice: { label: "Fyllning", options: ["lax", "avokado", "gurka"] } },
    "maki-8": { choice: { label: "Fyllning", options: ["lax", "avokado", "gurka"] } },
...
export function mixPrice(id, opts) {
  ...
  if (cfg && cfg.choice) {
    const pick = opts ? String(opts.pick || "") : "";
    if (!cfg.choice.options.includes(pick)) throw new Error(`Välj ${cfg.choice.label.toLowerCase()}.`);
    return { extra: 0, swaps: 0, detail: `${cfg.choice.label}: ${pick}` };
  }
  if (cfg && cfg.addon) {

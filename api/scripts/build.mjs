...
/* Kategoribilder i static/meny/<id>-{s,m,l}.{webp,jpg}. Saknas bilden får kategorin ingen banderoll. */
const IMG_ALT = {
...
const hasImg = id => existsSync(`static/meny/${id}-l.webp`);
const banner = c => hasImg(c.id) ? `
  <figure class="cat-img">
    <picture>
      <source type="image/webp" srcset="/meny/${c.id}-s.webp 480w, /meny/${c.id}-m.webp 800w, /meny/${c.id}-l.webp 1400w" sizes="(max-width:760px) 100vw, min(100vw - 48px, 1180px)">
      <img src="/meny/${c.id}-m.jpg" srcset="/meny/${c.id}-s.jpg 480w, /meny/${c.id}-m.jpg 800w, /meny/${c.id}-l.jpg 1400w" sizes="(max-width:760px) 100vw, min(100vw - 48px, 1180px)" width="1400" height="613" alt="${esc(IMG_ALT[c.id] || c.name)}" loading="lazy" decoding="async">
    </picture>
  </figure>` : "";
const square = id => hasImg(id) ? `<picture class="dish-img"><source type="image/webp" srcset="/meny/${id}-sq.webp"><img src="/meny/${id}-sq.jpg" width="640" height="640" alt="" loading="lazy" decoding="async"></picture>` : "";
